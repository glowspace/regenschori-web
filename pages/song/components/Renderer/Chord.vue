<template>
    <span class="chord"
        ...
        <div class="chord-sign"
             v-if="displayChordSign">
            ...
            <span class="chord-bass"
                  v-if="bass.length!==0">/{{bassNote}}</span>
            <span class="chord-right-bracket"
                  v-if="isOptional">)</span>
        </div><span :class="['chord-text', isDivided == 0 ? 'chord-text-spaced' : '']">
            <slot></slot>
        </span><span class="chord-line"
                     v-if="isDivided == 1"></span>
    </span>
</template>
