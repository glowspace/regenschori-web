<template>
  <div>
    <div class="mb-2" v-if="selected_tags.liturgy_part_tags.length">
      <h5>Část mše:</h5>
      <badge :title="tag.name"
             :key="tag.id"
             v-for="tag in selected_tags.liturgy_part_tags" class="mr-2"></badge>
    </div>

    <div class="mb-2" v-if="selected_tags.liturgy_period_tags.length">
      <h5>Liturgická doba:</h5>
      <badge :title="tag.name"
             :key="tag.id"
             v-for="tag in selected_tags.liturgy_period_tags" class="mr-2"></badge>
    </div>
...
<script>
   ...
   * Status of search.
   *
   * This widget shows which filters
   * user selects in search query.
   */
  export default {
    name: "SearchCurrentStatus",
    components: {Badge},
