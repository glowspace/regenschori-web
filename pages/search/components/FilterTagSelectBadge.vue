<template>
    <span class="badge" :class="{ disabled: !active }" @click="toggle()">{{
        tag.name
    }}</span>
</template>
...
export default {
    ...
    methods: {
        ...
        unselect() {
            ...
            if (this.tag.type_enum === 'LITURGY_PART') {
                this.selected_tags.liturgy_part_tags = this.selected_tags.liturgy_part_tags.filter(
                    obj => obj.id !== this.tag.id
                );
            }

            if (this.tag.type_enum === 'LITURGY_PERIOD') {
                this.selected_tags.liturgy_period_tags = this.selected_tags.liturgy_period_tags.filter(
                    obj => obj.id !== this.tag.id
                );
            }

            if (this.tag.type_enum === 'HISTORICAL_PERIOD') {
                this.selected_tags.historical_period_tags = this.selected_tags.historical_period_tags.filter(
                    obj => obj.id !== this.tag.id
                );
            }

            if (this.tag.type_enum === 'INSTRUMENTATION') {
                this.selected_tags.instrumentation_tags = this.selected_tags.instrumentation_tags.filter(
                    obj => obj.id !== this.tag.id
                );
            }
        }
